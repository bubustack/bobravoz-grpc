...
var (
	...
	connectorDownstreamDropCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			...
			Help: "Monotonic counter tracking downstream frames dropped by the data stream.",
		},
		[]string{"namespace", "binding", "reason"},
	)
	...
	connectorConnectionState = prometheus.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "bobravoz_connector_connection_state",
			Help: "Current connection state of the connector (1=connected, 0=disconnected).",
		},
		[]string{"hub_endpoint"},
	)
	connectorPatchSuccessCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "bobravoz_connector_patch_success_total",
			Help: "Monotonic counter tracking successful TransportBinding status patches.",
		},
		[]string{"namespace", "binding"},
	)
...
// RecordConnectorConnectionState sets the connection state gauge for the given hub endpoint.
// Pass connected=true when the connector establishes a connection, false when disconnected.
func RecordConnectorConnectionState(endpoint string, connected bool) {
	val := float64(0)
	if connected {
		val = 1
	}
	connectorConnectionState.WithLabelValues(endpoint).Set(val)
}
...
func RecordConnectorPatchSuccess(namespace, binding string) {
	connectorPatchSuccessCounter.WithLabelValues(namespace, binding).Inc()
}

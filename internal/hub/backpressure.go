...
const transportDefaultsTTL = 1 * time.Minute
...
func resolveTransportSettingsUncached(ctx context.Context, reader client.Reader, story *bubuv1alpha1.Story, transportName string) *transportSettings {
	...
	if err := json.Unmarshal(settings.Raw, &parsed); err != nil {
		log.Log.WithName("backpressure").V(1).Error(err, "Failed to parse transport settings", "transport", transportName)
		return nil
	}
	return &parsed
}
...
func resolveRecordingPolicyForStep(ctx context.Context, reader client.Reader, story *bubuv1alpha1.Story, step *bubuv1alpha1.Step) recordingPolicy {
	policy := recordingPolicy{mode: "off", sampleRate: 0}
	if step == nil {
		return policy
	}
	parsed := resolveTransportSettings(ctx, reader, story, step.Transport)
	...
	if policy.mode != "off" && policy.sampleRate == 0 {
...
func resolveStoryTransportSettings(ctx context.Context, reader client.Reader, story *bubuv1alpha1.Story, transportName string) *runtime.RawExtension {
	...
	if err != nil {
		log.Log.WithName("backpressure").V(1).Error(err, "Failed to encode story transport streaming settings", "transport", transportName)
	...
	if err != nil {
		log.Log.WithName("backpressure").V(1).Error(err, "Failed to merge transport settings", "transportRef", ref, "transport", transportName)
		return settings
	}
	if len(mergedRaw) == 0 {
		return settings
...
func getTransportDefaultSettings(ctx context.Context, reader client.Reader, ref string, transportName string) *runtime.RawExtension {
	...
	if err != nil {
		log.Log.WithName("backpressure").V(1).Error(err, "Failed to encode transport streaming defaults", "transportRef", ref, "transport", transportName)
	...
	transportDefaultsCache.entries[ref] = transportDefaultsEntry{settings: cloneRawExtension(settings), fetchedAt: now}
	transportDefaultsCache.mu.Unlock()
	return settings
}

...
func NewMessageBuffer(storyRunName, stepName string) *MessageBuffer {
	return &MessageBuffer{
		messages:     make([]*transportpb.DataPacket, 0, MaxBufferSize),
...
func (mb *MessageBuffer) FlushWithSender(ctx context.Context, send func(*transportpb.DataPacket) error) int {
	mb.mu.Lock()
	defer mb.mu.Unlock()
	defer func() {
		metrics.RecordHubBufferSize(mb.storyRunName, mb.stepName, len(mb.messages))

...
//
// Deprecated: Flush sends directly via downstream.Send and is not protected by any per-stream
// serialization lock. Use FlushWithSenderAndPolicy instead, which accepts a sender function
// called while holding the appropriate serialization mutex. Flush is retained only for tests
// that use a mock gRPC stream.
func (mb *MessageBuffer) Flush(ctx context.Context, downstream transportpb.HubService_ProcessServer) int {
	mb.mu.Lock()
	defer mb.mu.Unlock()
	defer func() {
		metrics.RecordHubBufferSize(mb.storyRunName, mb.stepName, len(mb.messages))
	...
	for i, msg := range mb.messages {
		...
		if err := downstream.Send(&transportpb.ProcessResponse{Packet: msg}); err != nil {

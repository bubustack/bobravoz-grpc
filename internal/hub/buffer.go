...
func (mb *MessageBuffer) ApplyLimits(limits bufferLimits) {
	...
	mb.trimToLimitsLocked("apply_limits")
	mb.trimLaneLimitsLocked("apply_limits")
}
...
// The lock is released during sends to prevent head-of-line blocking when a
// consumer is slow.
func (mb *MessageBuffer) FlushWithSenderAndPolicy(ctx context.Context, send func(*transportpb.DataPacket) error, allow func(*transportpb.DataPacket) bool) (int, error) {
	// Phase 1: Lock, evict expired, snapshot messages to send, unlock.
	mb.mu.Lock()
	mb.evictExpiredLocked(time.Now())

	if len(mb.messages) == 0 {
		metrics.RecordHubBufferSize(mb.storyRunName, mb.stepName, 0)
		metrics.RecordHubBufferBytes(mb.storyRunName, mb.stepName, mb.totalBytes)
		mb.mu.Unlock()
		return 0, nil
	}

	// Snapshot: copy the slice header so we can iterate without the lock.
	toSend := make([]*transportpb.DataPacket, len(mb.messages))
	copy(toSend, mb.messages)
	toSendTimes := make([]time.Time, len(mb.arrivalTimes))
	copy(toSendTimes, mb.arrivalTimes)
	mb.mu.Unlock()

	// Phase 2: Send messages without holding the lock.
	flushed := 0
	stopIndex := len(toSend) // index at which we stopped sending
	var flushErr error
	...
	if flushed == len(toSend) {
		stopIndex = len(toSend)
	}

	// Phase 3: Lock again to update state.
	mb.mu.Lock()
	// Deduct bytes and lane counts for successfully sent messages.
	for i := 0; i < flushed; i++ {
		msgSize := pbproto.Size(toSend[i])
		mb.totalBytes -= msgSize
		mb.decrementLaneLocked(laneForPacket(toSend[i]), msgSize)
	}

	// Rebuild the message list: unsent snapshot messages + any new messages
	// that were added while we didn't hold the lock.
	var unsent []*transportpb.DataPacket
	var unsentTimes []time.Time
	if stopIndex < len(toSend) {
		unsent = toSend[stopIndex:]
		if stopIndex < len(toSendTimes) {
			unsentTimes = toSendTimes[stopIndex:]
		}
	}

	// Messages added by concurrent Add() calls while we were sending sit
	// after the original snapshot length in mb.messages. Preserve them.
	// Guard against concurrent DropAll or trim that may have shortened the slice.
	var newMessages []*transportpb.DataPacket
	var newTimes []time.Time
	if len(mb.messages) > len(toSend) {
		newMessages = mb.messages[len(toSend):]
	}
	if len(mb.arrivalTimes) > len(toSend) {
		newTimes = mb.arrivalTimes[len(toSend):]
	}

	if len(unsent)+len(newMessages) == 0 {
		mb.messages = mb.messages[:0]
		mb.arrivalTimes = mb.arrivalTimes[:0]
	} else {
		combined := make([]*transportpb.DataPacket, 0, len(unsent)+len(newMessages))
		combined = append(combined, unsent...)
		combined = append(combined, newMessages...)
		mb.messages = combined

		combinedTimes := make([]time.Time, 0, len(unsentTimes)+len(newTimes))
		combinedTimes = append(combinedTimes, unsentTimes...)
		combinedTimes = append(combinedTimes, newTimes...)
		mb.arrivalTimes = combinedTimes
	}

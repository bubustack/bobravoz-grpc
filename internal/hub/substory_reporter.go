...
func (r *substoryReporter) watchFireAndForget(parentName, parentNamespace, childName, stepID string) {
	if r == nil || parentName == "" || childName == "" {
		return
	}
	go r.watchChild(parentName, parentNamespace, childName, stepID)
}

func (r *substoryReporter) watchChild(parentName, parentNamespace, childName, stepID string) {
	ctx, cancel := context.WithTimeout(context.Background(), substoryWatchTimeout)
	defer cancel()
	...
	for {
		...
		if err := r.client.Get(ctx, childKey, &child); err != nil {
			...
			r.log.V(1).Info("Substory watcher: failed to get child",
				"parent", parentName, "child", childName, "error", err)
			continue
		}
...
func (r *substoryReporter) patchParentDegraded(parentName, parentNamespace, condReason, message string) {
	...
	if err := r.client.Get(ctx, key, &parent); err != nil {
		r.log.V(1).Info("Substory reporter: parent StoryRun not found",
			"storyRun", key, "error", err)
		return
	}
	...
	if err := r.client.Status().Patch(ctx, &parent, client.MergeFrom(original)); err != nil {
		r.log.V(1).Info("Substory reporter: failed to patch parent StoryRun",
			"storyRun", key, "error", err)
		return
	}

...
func (c *templateSchemaCache) getEntry(ctx context.Context, templateName string) (templateSchemaEntry, error) {
	...
	c.mu.Lock()
	if entry, found := c.cache[templateName]; found {
		if time.Since(entry.createdAt) < templateSchemaCacheTTL {
			c.mu.Unlock()
			return entry, nil
		}
	...
	if template.Spec.InputSchema != nil && len(template.Spec.InputSchema.Raw) > 0 {
		compiled, err := compileSchema(template.Spec.InputSchema.Raw, entry.inputSchemaName)
		if err != nil {
			return templateSchemaEntry{}, err
		}
		entry.inputSchema = compiled
		entry.inputSchemaRaw = template.Spec.InputSchema.Raw
		entry.hasInputSchema = compiled != nil
	}

	if template.Spec.OutputSchema != nil && len(template.Spec.OutputSchema.Raw) > 0 {
		compiled, err := compileSchema(template.Spec.OutputSchema.Raw, entry.outputSchemaName)
		if err != nil {
			return templateSchemaEntry{}, err
		}

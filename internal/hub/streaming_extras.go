...
var (
	...
	sampleRand               = mathrand.New(mathrand.NewSource(time.Now().UnixNano()))
...
func applyRedactions(packetMap map[string]any, fields []string) {
	...
	for _, field := range fields {
		...
		rel := path
		if strings.HasPrefix(rel, "packet.") {
			rel = strings.TrimPrefix(rel, "packet.")
		}

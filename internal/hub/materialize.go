...
//
// The main entry points are routeToMaterialize (request path) and
// handleMaterializeResult (response path). Helper functions detect offloaded
// storage refs in step output expressions and build the intermediate request
// structures. The detectOffloadedOutputRefs function walks template expressions
// to find step output references that point to storage-backed data, which
// triggers the materialization detour.
package hub

...
const (
	...
	defaultFlowControlTimeout = 5 * time.Second
...
// getStream loads a stream entry by key from the active streams map.
func (sm *StreamManager) getStream(key string) (*streamEntry, bool) {
...
// getShadow loads a shadow stream entry by key from the shadow streams map.
func (sm *StreamManager) getShadow(key string) (*streamEntry, bool) {
	val, ok := sm.shadows.Load(key)
	if !ok {
		return nil, false
	}
	entry, ok := val.(*streamEntry)
...
// getState loads a stream state by key from the states map.
func (sm *StreamManager) getState(key string) (*streamState, bool) {
	val, ok := sm.states.Load(key)
	if !ok {
		return nil, false
	}
	state, ok := val.(*streamState)
	return state, ok
}
...
func (sm *StreamManager) HasStream(storyRunName, storyRunNamespace, stepID string) bool {
	...
	_, ok := sm.getStream(key)
	return ok
}

// TODO(security): Add connection rate limiting (token bucket) to prevent
// rapid connect/disconnect attacks from exhausting goroutines. See audit
// finding: unbounded goroutine creation per stream.

// AddStream adds a new stream to the manager and returns its entry handle.
// When a positive connector generation higher than the current active stream arrives,
// it is stored as a shadow (green) stream for blue-green handoff instead of replacing
// the active one.
//
//nolint:gocyclo // Stream registration coordinates blue-green handoff, buffering state, and replay setup.
...
func (sm *StreamManager) RemoveStream(storyRunName, storyRunNamespace, stepID string, entry *streamEntry) {
	...
	if shadowEntry, ok := sm.getShadow(key); ok {
		if shadowEntry == entry {
			sm.shadows.Delete(key)
			sm.activeCount.Add(-1)
			sm.log.Info("Shadow stream removed", "key", key)
			// If active stream is still running, return to ready.
			if _, hasActive := sm.getStream(key); hasActive {
				if state, ok := sm.getState(key); ok {
					state.mu.Lock()
	...
	if activeEntry, ok := sm.getStream(key); ok {
		if activeEntry == entry {
			...
			// Clean up state if no active or shadow stream remains for this key.
			_, hasActive := sm.getStream(key)
			_, hasShadow := sm.getShadow(key)
			if !hasActive && !hasShadow {
...
func (sm *StreamManager) CutoverShadow(storyRunName, storyRunNamespace, stepID string) bool {
	...
	shadowVal, hasShadow := sm.shadows.LoadAndDelete(key)
	if !hasShadow {
	...
	oldEntry, hadOld := sm.getStream(key)
...
//
//nolint:gocyclo // Send-or-buffer enforces flow, buffering, and stream handoff rules in one dispatch path.
func (sm *StreamManager) SendOrBufferWithOptions(ctx context.Context, storyRunName, storyRunNamespace, stepID string, packet *transportpb.DataPacket, opts StreamOptions) bool {
	...
	if entry, ok := sm.getStream(key); ok {
		stream := entry.stream
...
//nolint:gocyclo // Policy application keeps flow-control, delivery, and buffer reconciliation in one state mutation.
...
func (s *streamState) setHandoff(phase, reason string) {
	...
	if cb != nil {
		go func() {
			defer func() {
				if r := recover(); r != nil {
					log.Log.Error(fmt.Errorf("handoff callback panicked: %v", r), "recovered from panic in handoff callback")
				}
			}()
			cb(storyRunName, storyRunNamespace, stepID, phase, reason)
		}()
	}
}
...
//nolint:gocyclo // Envelope assignment must preserve replay, partitioning, and downstream sequencing semantics together.
...
//nolint:gocyclo // Send admission combines lifecycle checks, in-flight limits, and replay semantics in one gate.
...
//nolint:gocyclo // Sent-state tracking updates checkpoints, replay buffers, and delivery counters together.
	...
	if drainEnabled && state != nil {
		...
		if shouldApplyDrainTimeout(reason) {
			if state.setDrainPaused(true) {
				sm.sendFlow(key, &transportpb.FlowControl{Signal: transportpb.FlowControlSignal_FLOW_CONTROL_SIGNAL_PAUSE})
			}
		}
	}
...
func (sm *StreamManager) updatePauseResume(key string, buffer *MessageBuffer, state *streamState) {
	...
	if paused && resumeAt > 0 && utilization <= resumeAt {
		...
		state.mu.Unlock()
		if !drainPaused {
			sm.sendFlow(key, &transportpb.FlowControl{Signal: transportpb.FlowControlSignal_FLOW_CONTROL_SIGNAL_RESUME})
		}
	}
}
...
func (sm *StreamManager) sendFlow(key string, flow *transportpb.FlowControl) {
	...
	entry, ok := sm.getStream(key)
	if !ok || entry.stream == nil {
		return
	}
	sendCtx := safeStreamContext(entry.stream.grpcStream, entry.stream.fallbackCtx)
	if timeout := sm.flowControlTimeout(); timeout > 0 {
		var cancel context.CancelFunc
		sendCtx, cancel = context.WithTimeout(sendCtx, timeout)
		defer cancel()
	}
	if err := entry.stream.SendFlow(sendCtx, flow); err != nil {
		sm.log.V(1).Info("Failed to send flow control update", "key", key, "error", err)
	}
}

func (sm *StreamManager) flowControlTimeout() time.Duration {
	if sm == nil {
		return defaultFlowControlTimeout
	}
	switch {
	case sm.perMessageTimeout <= 0:
		return defaultFlowControlTimeout
	case sm.perMessageTimeout < defaultFlowControlTimeout:
		return sm.perMessageTimeout
	default:
		return defaultFlowControlTimeout
	}
}
...
//
//nolint:gocyclo // Flow application reconciles credits, ACK state, and buffered delivery across active/shadow streams.
...
//nolint:gocyclo // Replay walks multiple delivery states while preserving ordering and cancellation semantics.
...
//nolint:gocyclo // Checkpoint restore rebuilds replay state, ACK windows, and durable markers from persisted data.
...
//nolint:gocyclo // Checkpoint persistence aggregates live delivery state into a durable snapshot.
func (sm *StreamManager) maybeCheckpoint(state *streamState) {
	...
	defer writeCancel()
	if err := sm.storage.WriteBlob(writeCtx, checkpointPath(state), "application/json", payload); err != nil {
...
func (sm *StreamManager) flushBuffers(ctx context.Context) {
	...
	sm.buffers.Range(func(key, value any) bool {
		...
		keyStr, _ := key.(string)
		entry, ok := sm.getStream(keyStr)
		if !ok || entry == nil || entry.stream == nil {
			return true
		}
		state, _ := sm.getState(keyStr)
		baseCtx := ctx
		if baseCtx == nil {
			baseCtx = context.Background()
		}

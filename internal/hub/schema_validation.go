...
// validateJSONAgainstSchema validates raw JSON bytes against a compiled schema
// and returns a descriptive error when validation fails.
...
// compileSchema normalizes raw schema bytes and compiles them into a reusable
// gojsonschema.Schema.
...
func normalizeSchemaBytes(schema []byte) ([]byte, error) {
	...
	nodeCount := 0
	normalized, err := normalizeSchemaNode(root, 0, &nodeCount)
	if err != nil {
		return nil, err
	}
...
func normalizeSchemaNode(node any, depth int, nodeCount *int) (any, error) {
	if depth > maxSchemaNormalizationDepth {
		return nil, fmt.Errorf("schema exceeds max normalization depth %d", maxSchemaNormalizationDepth)
	}
	*nodeCount++
	if *nodeCount > maxSchemaNormalizationNodes {
		return node, nil
	}
	switch typed := node.(type) {
	case map[string]any:
		normalized, err := normalizeObjectSchema(typed, depth, nodeCount)
		if err != nil {
			return nil, err
		}
		return allowStorageRefAlternative(normalized), nil
	case []any:
		for i := range typed {
			normalized, err := normalizeSchemaNode(typed[i], depth+1, nodeCount)
			if err != nil {
				return nil, err
			}
			typed[i] = normalized
...
func normalizeObjectSchema(obj map[string]any, depth int, nodeCount *int) (map[string]any, error) {
	requiredSet, err := liftInlineRequiredFlags(obj, depth, nodeCount)
	if err != nil {
		return nil, err
	}
	mergeRequiredSet(obj, requiredSet)
	if err := normalizeNestedSchemaLocations(obj, depth, nodeCount); err != nil {
		return nil, err
	}
	return obj, nil
}
...
const (
	maxSchemaDefaultDepth       = 32
	maxSchemaNormalizationDepth = maxSchemaDefaultDepth
	maxSchemaNormalizationNodes = 10000
)
...
//nolint:gocyclo // Default propagation needs to keep the recursive schema branches together.
...
func liftInlineRequiredFlags(obj map[string]any, depth int, nodeCount *int) (map[string]struct{}, error) {
	...
	for propName, rawChild := range props {
		...
		normalized, err := normalizeSchemaNode(cleaned, depth+1, nodeCount)
		if err != nil {
			return nil, err
		}
...
func normalizeNestedSchemaLocations(obj map[string]any, depth int, nodeCount *int) error {
	if err := normalizeItemsNode(obj, depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeSingleSchemaField(obj, "additionalProperties", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeMapOfSchemas(obj, "patternProperties", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeSchemaSlice(obj, "allOf", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeSchemaSlice(obj, "anyOf", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeSchemaSlice(obj, "oneOf", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeMapOfSchemas(obj, "definitions", depth, nodeCount); err != nil {
		return err
	}
	if err := normalizeMapOfSchemas(obj, "$defs", depth, nodeCount); err != nil {
		return err
	}
	return normalizeSingleSchemaField(obj, "not", depth, nodeCount)
}

func normalizeItemsNode(obj map[string]any, depth int, nodeCount *int) error {
	items, has := obj["items"]
	if !has {
		return nil
	}
	...
	case map[string]any, []any:
		normalized, err := normalizeSchemaNode(typed, depth+1, nodeCount)
		if err != nil {
			return err
		}
...
func normalizeSingleSchemaField(obj map[string]any, key string, depth int, nodeCount *int) error {
	if raw, has := obj[key]; has {
		if schemaMap, ok := raw.(map[string]any); ok {
			normalized, err := normalizeSchemaNode(schemaMap, depth+1, nodeCount)
			if err != nil {
				return err
			}
			obj[key] = normalized
...
func normalizeMapOfSchemas(obj map[string]any, key string, depth int, nodeCount *int) error {
	raw, has := obj[key].(map[string]any)
	if !has {
		return nil
	}
	for k, v := range raw {
		normalized, err := normalizeSchemaNode(v, depth+1, nodeCount)
		if err != nil {
			return err
		}
		raw[k] = normalized
...
func normalizeSchemaSlice(obj map[string]any, key string, depth int, nodeCount *int) error {
	raw, has := obj[key].([]any)
	if !has {
		return nil
	}
	for i := range raw {
		normalized, err := normalizeSchemaNode(raw[i], depth+1, nodeCount)
		if err != nil {
			return err
		}
		raw[i] = normalized

...
//
//nolint:gocyclo // Reconciliation keeps guard clauses, condition updates, and transport dispatch in one flow.
func (r *TransportReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// This controller only cares about realtime patterns
	if !story.Spec.Pattern.IsRealtime() {
		return ctrl.Result{}, nil
	}
...
//
//nolint:gocyclo // Normal reconciliation coordinates finalizers, status changes, transport execution, and error mapping.
func (r *TransportReconciler) reconcileNormal(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, transportImpl transport.Transport, transportType string, cm *bobrapetconditions.ConditionManager) (ctrl.Result, error) {
	...
	patch := client.MergeFrom(storyRun.DeepCopy())
	setTransportConditions(cm, &storyRun.Status.Conditions, false, bobrapetconditions.ReasonReconciling, "Reconciling transport")
	...
	if err := transportImpl.Reconcile(ctx, storyRun, story); err != nil {
		...
		patch := client.MergeFrom(storyRun.DeepCopy())
		setTransportConditions(cm, &storyRun.Status.Conditions, false, bobrapetconditions.ReasonTransportFailed, err.Error())
		if err := r.Status().Patch(ctx, storyRun, patch); err != nil {
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, err
	...
	patch = client.MergeFrom(storyRun.DeepCopy())
	setTransportConditions(cm, &storyRun.Status.Conditions, true, bobrapetconditions.ReasonTransportReady, "Transport is ready")
	if err := r.Status().Patch(ctx, storyRun, patch); err != nil {
		return ctrl.Result{}, err
	}
	if r.Recorder != nil {
...
func (r *TransportReconciler) reconcileDelete(ctx context.Context, storyRun *runsv1alpha1.StoryRun, story *bubuv1alpha1.Story, transportImpl transport.Transport, transportType string, cm *bobrapetconditions.ConditionManager) (ctrl.Result, error) {
	...
	patch := client.MergeFrom(storyRun.DeepCopy())
	setTransportConditions(cm, &storyRun.Status.Conditions, false, bobrapetconditions.ReasonCleaningUp, fmt.Sprintf("Cleaning up %s transport", transportType))
	if err := r.Status().Patch(ctx, storyRun, patch); err != nil {
		return ctrl.Result{}, err
	}
	...
	patch = client.MergeFrom(storyRun.DeepCopy())
	setTransportConditions(cm, &storyRun.Status.Conditions, false, transportCleanedReason, fmt.Sprintf("%s transport cleanup completed", transportType))
	if err := r.Status().Patch(ctx, storyRun, patch); err != nil {
		return ctrl.Result{}, err
	}
	if r.Recorder != nil {
...
func setTransportConditions(cm *bobrapetconditions.ConditionManager, conditions *[]metav1.Condition, ready bool, reason, message string) {
	cm.SetReadyCondition(conditions, ready, reason, message)
	status := metav1.ConditionFalse
	if ready {
		status = metav1.ConditionTrue
	}
	cm.SetCondition(conditions, bobrapetconditions.ConditionTransportReady, status, reason, message)
}
...
func storyRunTransportRelevantPredicate() predicate.Predicate {
	return predicate.Funcs{
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			// Keep retrying deletion while this controller's finalizer is still
			// present. Otherwise a missed deletion-edge event can leave StoryRuns
			// stuck forever with transportFinalizer.
			if newRun.GetDeletionTimestamp() != nil && controllerutil.ContainsFinalizer(newRun, transportFinalizer) {
				return true
			}
...
func calculateBindingPendingRequeue(waitDuration time.Duration) time.Duration {
	...
	const (
		secondsPerBackoffInterval = 10
		maxBackoffExponent        = 4
	)
	// Calculate how many intervals have passed, capped to prevent overflow.
	intervals := min(int(waitDuration.Seconds()/secondsPerBackoffInterval), maxBackoffExponent)
